...
// Trang thai ir slot
void Read_Sensor() {
    S1 = digitalRead(irCar6) ? 0 : 1;
    S2 = digitalRead(irCar1) ? 0 : 1;  
    S3 = digitalRead(irCar2) ? 0 : 1;
    S4 = digitalRead(irCar3) ? 0 : 1;
    S5 = digitalRead(irCar4) ? 0 : 1;
    S6 = digitalRead(irCar5) ? 0 : 1;
}
...
void handleGate(int &slot, int &isCardVao, int &isCardRa) {
  // int isCardVao = 0;
  // int isCardRa = 0;
  // Mở cổng vào nếu còn slot và phát hiện xe vào
  if (slot > 0 && digitalRead(irEnter) == 1 && digitalRead(irExit) == 0 && isCardVao == 1) {
    slot -= 1;
    Serial1.println("SERVO 90"); // 1 vao, 2 ra, 90 mo, 0 dong
  } 
  
  // Mở cổng ra nếu phát hiện xe ra
  if (digitalRead(irExit) == 1 && digitalRead(irEnter) == 0 && isCardRa == 1) {
    slot += 1;
    Serial2.println("SERVO 90");
  }

  // Trường hợp bãi đầy nhưng vẫn phát hiện xe ra
  if (slot == 0 && digitalRead(irExit) == 1) {
